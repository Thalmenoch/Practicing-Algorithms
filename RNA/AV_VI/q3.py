...
# Função para gerar dados para a porta NOR
def gerando_nor():
    X, y = make_classification(n_samples=1000, n_features=2, n_informative=2, n_redundant=0, n_clusters_per_class=1, random_state=42)
    y = np.logical_not(y)  # Inverte as classes para representar a porta NOR
    return X, y

# Função para treinar a RNA RBF
def treinando_rbf(X_train, y_train, n_clusters=10):
    ...
    rbf_mlp = MLPClassifier(hidden_layer_sizes=(n_clusters,), activation='logistic', max_iter=1000, random_state=42)
...
if __name__ == "__main__":
    ...
    # Treinar a RNA RBF
    centers, rbf_mlp = treinando_rbf(X_train, y_train)
